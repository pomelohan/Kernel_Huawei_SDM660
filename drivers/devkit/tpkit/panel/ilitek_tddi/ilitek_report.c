...
static int ilitek_report_touch_data(struct ts_fingers *p_info)
{
    ...
    for (i = 0; i < ILITEK_TOUCSCEEN_FINGERS; i++) {
        ...
        /* fix can't draw line along the edge */
        if (g_ilitek_rpt->fingers[i].x == g_ilitek_ts->x_min) {
            g_ilitek_rpt->fingers[i].x += 1;
        } else if (g_ilitek_rpt->fingers[i].y == g_ilitek_ts->y_min) {
            g_ilitek_rpt->fingers[i].y += 1;
        } else if (g_ilitek_rpt->fingers[i].x == g_ilitek_ts->x_max) {
            g_ilitek_rpt->fingers[i].x -= 1;
        } else if (g_ilitek_rpt->fingers[i].y == g_ilitek_ts->y_max) {
            g_ilitek_rpt->fingers[i].y -= 1;
        }
