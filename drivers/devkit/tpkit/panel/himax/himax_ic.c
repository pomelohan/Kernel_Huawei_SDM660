...
void himax_get_rawdata_from_event(int RawDataLen,int hx_touch_info_size, int mul_num, int sel_num, int index, uint8_t *buf )
{
	...
	if (hx_selftest_flag == 1) {
		TS_LOG_INFO("%s:Entering get raw from event stack !\n", __func__);
		temp1 = ((mul_num * sel_num + mul_num + sel_num)/(RawDataLen));
		temp2 = ((mul_num * sel_num + mul_num + sel_num)%(RawDataLen));
		rawdata_frame_size = temp2 > 0 ? temp1 + 1 : temp1;
	}
	...
	for (i = 0; i < RawDataLen; i++)
	{
		...
		{ //mutual
			...
			if (hx_selftest_flag == 1)
			{
				...
				now_raw_frame_idx = buf[hx_touch_info_size - 1];
				g_raw_data_chk_arr[now_raw_frame_idx] = true;
			}
		}
		else
		{//self
	...
	{
		cnt = 0;
		for(i = 0; i < rawdata_frame_size; i++)
		{
			if(g_raw_data_chk_arr[i] == true)
				cnt++;
		}
		if (cnt == rawdata_frame_size)
		{
			TS_LOG_INFO("g_test_collect_counter = %d\n", g_test_collect_counter);
			g_test_collect_counter++;
		}
		if(g_test_collect_counter == TEST_DATA_TIMES)
		{
			for(i = 0; i < rawdata_frame_size; i++)
			{
				g_raw_data_chk_arr[i] = false;
			}
			g_test_collect_counter = 0;
			hx_selftest_flag = 0;
			msleep(HX_SLEEP_10MS);
		}
    }
