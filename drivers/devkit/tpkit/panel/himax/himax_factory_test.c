...
int himax_self_delta_test(int step)
{
	...
	for(index1 = 1;index2<rx_delta_num;index1++)
	{
		...
		if(g_himax_ts_data->p2p_test_sel) {
			hx_criteria[delta_up] = (uint8_t)p2p_on_rx_delta_limit_up[index1 - 1];
		}
...
int himax_iir_test(int step) //for Noise Delta
{
	...
	for(index1=0;index1<(rx+tx);index1++)
	{
		...
		if(g_himax_ts_data->p2p_test_sel) {
			hx_criteria[iir_mutual] = (uint8_t)p2p_on_noise_self_limit[index1];
		}
...
void himax_print_rawdata(int mode)
{
	...
	uint16_t x_channel = getXChannel();
	uint16_t y_channel = getYChannel();
	uint8_t self_buff[HX_RECEIVE_BUF_MAX_SIZE]  = {0};
...
{
	uint8_t command_F1h_bank[2] = {0xF1, 0x02};
	uint8_t command_F1h_iir[2] = {0xF1, 0x01};
	uint8_t command_F1h_back[2] = {0xF1, 0x00};
	uint8_t cmdbuf[4] = {0};
	int i = 0;
	uint16_t mutual_num = getXChannel() * getYChannel();
	uint16_t self_num = getXChannel() + getYChannel();
	int retval = NO_ERR;

	TS_LOG_INFO("%s:Start get raw data\n", __func__);


	TS_LOG_INFO(" mutual_num = %d, self_num = %d \n",mutual_num, self_num);
	TS_LOG_INFO(" hx_selftest_flag = %d \n",hx_selftest_flag);


	/* 1. get bank */
	 hx_selftest_flag = 1;
	TS_LOG_INFO(" Write diag cmd = %d \n",command_F1h_bank[1]);
	...
	for (i = 0; i < 100; i++) //check for max 100 times//collect data
	{
		TS_LOG_INFO(" conuter = %d ,hx_selftest_flag = %d\n",i,hx_selftest_flag);
		if(hx_selftest_flag == 0)
		break;
		msleep(10);
	}

	if (mutual_data && self_data){
		memcpy(mutual_bank,&mutual_data[0],mutual_num);
		memcpy(self_bank,&self_data[0],self_num);
	}
	else {
		TS_LOG_ERR("mutual_data/self_data is NULL!\n");
		retval = HX_ERROR;
		goto BUS_FAIL_END;
	}
	...
	TS_LOG_INFO("Back to Normal from bank!\n");
	i2c_himax_write(command_F1h_back[0],&command_F1h_back[1], 1, sizeof(command_F1h_back), DEFAULT_RETRY_CNT);
	retval = i2c_himax_write(HX_CMD_TSSON,&cmdbuf[0], 0, sizeof(cmdbuf), DEFAULT_RETRY_CNT);
	if( retval != 0 ) {
		TS_LOG_INFO("I2C Fail!\n");
		goto BUS_FAIL_END;
	...
	i2c_himax_write(command_F1h_iir[0],&command_F1h_iir[1], 1, sizeof(command_F1h_iir), DEFAULT_RETRY_CNT);
	retval = i2c_himax_write(HX_CMD_TSSON,&cmdbuf[0], 0, sizeof(cmdbuf), DEFAULT_RETRY_CNT);
	if( retval != 0 ) {
		TS_LOG_INFO("I2C Fail!\n");
		goto BUS_FAIL_END;
	...
	for (i = 0; i < 10; i++) //check for max 100 times//collect data
	{
		TS_LOG_INFO(" conuter = %d ,hx_selftest_flag = %d\n",i,hx_selftest_flag);
		if(hx_selftest_flag == 0)
			break;
	...
	TS_LOG_INFO("Back to Normal from iir!\n");
	retval = i2c_himax_write(HX_CMD_TSSOFF,&cmdbuf[0], 0, sizeof(cmdbuf), DEFAULT_RETRY_CNT);
	if( retval != 0 ) {
		TS_LOG_INFO("I2C Fail!\n");
		goto BUS_FAIL_END;
...
static uint8_t hx852xes_self_test(int step)
{
	...
	if (fail_flag)
	{
		result = 0xF1;     //Fail ID for Condition 1
	...
	else
	{
		...
		if (fail_flag)
		{
			result = 0xF2; //Fail ID for Condition 2
			TS_LOG_INFO(" Fail = 0x%02X \n", result);
			goto END_FUNC;
		}
		else
		{
			...
			if (fail_flag)
			{
				result = 0xF3; //Fail ID for Condition 3
				TS_LOG_INFO(" Fail = 0x%02X \n", result);
				goto END_FUNC;
			}
			else
...
#ifdef HX_FAC_LOG_PRINT
	//=====debug log======
	for (index1 = 0; index1 < (mutual_num + self_num) * 10; ) {
		if ((index1 + 13) >  (mutual_num + self_num) * 10)
			break;
		printk("%4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d \n", fac_dump_buffer[index1 + 0],fac_dump_buffer[index1 + 1],
				fac_dump_buffer[index1 + 2],fac_dump_buffer[index1 + 3],fac_dump_buffer[index1 + 4],fac_dump_buffer[index1 + 5],
				fac_dump_buffer[index1 + 6],fac_dump_buffer[index1 + 7],fac_dump_buffer[index1+8],fac_dump_buffer[index1 + 9],
				fac_dump_buffer[index1 + 10],fac_dump_buffer[index1 + 11],fac_dump_buffer[index1+12],fac_dump_buffer[index1 + 13]);
		index1 += 13;
	}
	//=====debug log=====
#endif
